...
def test_worktree_manager_forces_stale_existing_branch_after_prune(tmp_path: Path, monkeypatch) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    root = tmp_path / "workspaces"
    calls: list[list[str]] = []
    ...
    def fake_run(command: list[str], *, cwd: Path, text: bool, capture_output: bool, check: bool) -> Any:
        ...
        if command == ["git", "worktree", "prune"]:
            return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
        if command[:4] == ["git", "worktree", "add", "-b"]:
            return subprocess.CompletedProcess(command, 128, stdout="", stderr="fatal: a branch named 'codex-fleet/CF-1' already exists")
        if command[:4] == ["git", "worktree", "add", "-f"]:
            return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
    ...
    assert workspace.created_now is True
    assert workspace.path == root / "repo" / "CF-1"
    assert ["git", "worktree", "add", "-f", str(root / "repo" / "CF-1"), "codex-fleet/CF-1"] in calls
