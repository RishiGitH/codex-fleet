...
def test_test_proof_records_transcript_summary_when_no_runnable_target(tmp_path: Path) -> None:
    result = test_proof.run_test_proof(tmp_path, run_id="run-1")

    assert result.status == "transcript_only"
    assert result.proof_kind == "transcript_only"
    assert result.preview_url is None
    assert result.artifacts[0].name == "proof-summary.txt"


def test_detect_proof_target_static_html(tmp_path: Path) -> None:
    (tmp_path / "index.html").write_text("<h1>Hello</h1>\n")

    target = test_proof.detect_proof_target(tmp_path)

    assert target.kind == "browser_static"


def test_detect_proof_target_node_dev(tmp_path: Path) -> None:
    (tmp_path / "package.json").write_text('{"scripts":{"dev":"vite --host 127.0.0.1"}}\n')

    target = test_proof.detect_proof_target(tmp_path)

    assert target.kind == "browser_dev"


def test_detect_proof_target_cli(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text("[project]\nname='demo'\n[tool.pytest.ini_options]\n")
    (tmp_path / "tests").mkdir()

    target = test_proof.detect_proof_target(tmp_path)

    assert target.kind == "cli"
    assert "pytest" in target.commands
...
def test_test_proof_installs_builds_previews_and_captures(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(test_proof, "_free_port", lambda: 4123)
...
def test_test_proof_static_html_previews_and_captures(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_start(command: str, workspace: Path, port: int, preview_log: Path) -> FakeProcess:
        captured_commands.append(command)
        preview_log.write_text("static preview\n")
        return FakeProcess()

    video = tmp_path / ".codex-fleet" / "artifacts" / "run-static" / "video.webm"
    desktop = tmp_path / ".codex-fleet" / "artifacts" / "run-static" / "desktop.png"
    mobile = tmp_path / ".codex-fleet" / "artifacts" / "run-static" / "mobile.png"
    ...
    result = test_proof.run_test_proof(tmp_path, run_id="run-static")

    assert result.status == "passed"
    assert result.proof_kind == "browser_video"
    assert result.video_path == video
    assert "http.server" in captured_commands[0]


def test_test_proof_cli_logs_when_backend_project(monkeypatch, tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text("[project]\nname='demo'\n[tool.pytest.ini_options]\n")
    (tmp_path / "tests").mkdir()
    calls: list[list[str]] = []
    ...
    def fake_runner(command: list[str], cwd: Path, timeout_seconds: int, artifact: Path) -> subprocess.CompletedProcess[str]:
        calls.append(command)
        artifact.write_text("tests passed\n")
        return subprocess.CompletedProcess(command, 0, stdout="tests passed\n", stderr="")

    result = test_proof.run_test_proof(tmp_path, run_id="run-cli", command_runner=fake_runner)

    assert result.status == "cli_passed"
    assert result.proof_kind == "cli_logs"
    assert calls == [["pytest"]]
