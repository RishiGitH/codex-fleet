...
The repo has a Python CLI, repo doctor, harness generator, memory tracker, Plane REST adapter, Plane state bootstrap, worktree manager, fake runner, Codex CLI runner as the default real runner, preserved Codex App Server runner boundary, SQLite run store, daemon loop, manual draft PR helper, Codex config, subagents, skills, token/context helpers, and local docs.
...
6. Test a real Codex CLI run, then keep App Server as the richer optional boundary.
