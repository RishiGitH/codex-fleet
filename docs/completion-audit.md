...
- Plane customizations now live directly in tracked `apps/plane`; patch export/apply support has been removed.
...
- Typecheck: `pnpm --dir apps/plane --filter web check:types` passed with only the existing module-type warning from Plane's tailwind package.
...
- `npm pack --dry-run`: package contains CLI, docs, assets, and `src/codex_fleet/plane_local_bootstrap.py`; Python cache files are excluded.
...
| Ensure local Plane is available | Done | Branded fork source exists at `apps/plane` and passes `plane-verify`. Official self-host runtime exists at `.codex-fleet/plane-selfhost`; `plane-status` reports Docker daemon ready and Plane ready at `http://127.0.0.1:8080`. |
| Require tracked Plane source | Done | `apps/plane` is required; stale `.codex-fleet/plane-src` blocks startup with instructions to delete it. `start_plane` installs/starts official self-host Plane under `.codex-fleet/plane-selfhost`. |
| Pinned Plane source strategy | Done | The default Plane source pin is a packaged release artifact at `src/codex_fleet/resources/plane-source.lock.yml`; `apps/plane/.codex-fleet-plane-source.yml` records the upstream source and pinned commit. |
