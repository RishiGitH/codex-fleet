...
const CODEX_FLEET_BOARD_STATES = ["Backlog", "Ready", "Planning", "Running", "Needs Input", "Human Review", "Done"];
const CODEX_FLEET_BOARD_STATE_ORDER = new Map(CODEX_FLEET_BOARD_STATES.map((state, index) => [state.toLowerCase(), index]));
...
export const KanBan = observer(function KanBan(props: IKanBan) {
  ...
  const visibleList =
    group_by === "state"
      ? list
          .filter((column) => CODEX_FLEET_BOARD_STATE_ORDER.has(String(column.name).toLowerCase()))
          .sort(
            (left, right) =>
              (CODEX_FLEET_BOARD_STATE_ORDER.get(String(left.name).toLowerCase()) ?? 99) -
              (CODEX_FLEET_BOARD_STATE_ORDER.get(String(right.name).toLowerCase()) ?? 99)
          )
      : list;
  ...
  return (
    <ContentWrapper className={`relative flex-row gap-4 !pt-2 !pb-0`}>
      {visibleList &&
        ...
        visibleList.map((subList: IGroupByColumn, groupIndex) => {
