    ...
    >
      <Tooltip
        ...
        tooltipContent={projectLabels
          ...
          .map((l) => friendlyCodexFleetLabel(l?.name ?? ""))
          .join(", ")}
        renderByDefault={false}
      >
...
const friendlyCodexFleetLabel = (name: string) =>
  ({
...
const LabelItem = observer(function LabelItem({
  ...
  const { t } = useTranslation();
  const labelName = friendlyCodexFleetLabel(label?.name ?? "");

  return (
    <Tooltip
      position="top"
      tooltipHeading={t("common.labels")}
      tooltipContent={labelName}
      isMobile={isMobile}
    ...
    >
      ...
      >
        <div className="flex max-w-full items-center gap-1.5 overflow-hidden text-secondary">
          ...
          />
          <div className="line-clamp-1 inline-block w-auto max-w-[200px] truncate">{labelName}</div>
        </div>
      </div>
    </Tooltip>
  );
